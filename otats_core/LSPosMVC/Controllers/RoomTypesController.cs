    ...
    public class RoomTypesController : ApiController
    {
        ...
        public HttpResponseMessage GetByHotel(int hotelId)
        {
            ...
            try
            {
                ...
                {
                    RoomTypesData db = new RoomTypesData();
                    var data = db.GetByHotel(hotelId, userinfo.ID_QLLH);
        ...
        [HttpPost]
        [Route("insert")]
        public HttpResponseMessage Insert(RoomTypesModel model)
        {
            HttpResponseMessage response = new HttpResponseMessage();
            response = Request.CreateResponse(HttpStatusCode.NotFound, Config.DU_LIEU_TOKEN_TRUY_CAP_KHONG_DUNG);
            try
        ...
        [HttpPost]
        [Route("update")]
        public HttpResponseMessage Update(RoomTypesModel model)
        {
            HttpResponseMessage response = new HttpResponseMessage();
            response = Request.CreateResponse(HttpStatusCode.NotFound, Config.DU_LIEU_TOKEN_TRUY_CAP_KHONG_DUNG);
            try
