angular.module('app').controller('bm040_BieuDoTyTrongDoanhThuTheoKhachHangController', function ($scope, $rootScope, $timeout, Notification, ComboboxDataService, bieuDoDoanhThuDataService) {
    CreateSiteMap();
    ...
    function listColumnsgrid() {
        var dataList = [];
        dataList.push({
            field: "tenLoaiKhachHang", title: $.i18n('header_loaikhachhang'), footerTemplate: $.i18n('label_total'), attributes: { style: "text-align: center" }, footerAttributes: { style: "text-align: center" }, headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, filterable: false, width: "150px"
        });
        dataList.push({
    ...
    function load_chart() {
        ...
        bieuDoDoanhThuDataService.getBieuDoTyTrongDoanhThuTheoKhachHang(fromdate, todate).then(function (response) {
    ...
    function loadgrid() {
        ...
        let fromdate = kendo.toString($scope.obj_TuNgay, formatDateTimeFilter);
        let todate = kendo.toString($scope.obj_DenNgay, formatDateTimeFilter);
        bieuDoDoanhThuDataService.getBaoCaoTyTrongDoanhThuTheoKhachHang(fromdate, todate).then(function (response) {
            $scope.gridData = {
                data: response.data,
                schema: {
                    model: {
    ...
    $scope.xuatExcel = function () {
        ...
        let fromdate = kendo.toString($scope.obj_TuNgay, formatDateTimeFilter);
        let todate = kendo.toString($scope.obj_DenNgay, formatDateTimeFilter);
        bieuDoDoanhThuDataService.getExcelTyTrongDoanhThuTheoKhachHang(fromdate, todate).then(function (result) {
            if (result.flag)
