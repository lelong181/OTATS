angular.module('app').controller('tonghopdichvutheobooking2Controller', function ($rootScope, $scope, $state, Notification, bieuDoDoanhThuDataService, ComboboxDataService) {
    CreateSiteMap();
    ...
    function listColumnsgrid() {
        ...
        dataList.push({
            field: "soLuong", title: "Số lượng (Click từng ô để kiểm tra trạng thái sử dụng vé)",
            //aggregates: ["sum"],
            template: function (e) {
                return "<p ng-click='checkused(this)' style='color:blue'>" + e.soLuong + "</p>";
            },
            //format: formatNumberInGrid($rootScope.UserInfo.dinhDangSo),
            //footerTemplate: function (e) {
            //    if (e.s_9950) {
            ...
            attributes: {
                ...
                title:"Bấm để kiểm tra trạng thái sử dụng vé!"
            }, headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, filterable: defaultFilterableGrid, width: "150px"
        });
        dataList.push({
    ...
    function loadgrid() {
        ...
        bieuDoDoanhThuDataService.gettonghopdichvutheobooking2(sitecode, fromdate, todate).then(function (result) {
            $scope.gridData = {
                data: result.data,
                schema: {
                    model: {
    ...
    $scope.checkused = function (item) {
        console.log(item);
        let soluong = item.dataItem.soLuong
        if (item.dataItem.soLuong.indexOf("/") > 0) {
            soluong = item.dataItem.soLuong.split('/')[1];
        }
        //item.dataItem.set("soLuong", item.dataItem.soLuong + '/' + item.dataItem.soLuong)
        kendo.ui.progress($("#grid"), true);
        bieuDoDoanhThuDataService.checkstatusticket(item.dataItem.siteCode, item.dataItem.bookingCode, item.dataItem.maDichVu).then(function (result) {
            console.log(result);
            if (result.data[0].daSuDung > 0) {
                item.dataItem.set("soLuong", result.data[0].daSuDung + '/' + soluong);
            } else {
                item.dataItem.set("soLuong", 0 + '/' + soluong);
            }
            kendo.ui.progress($("#grid"), false);
        })
    }
