angular.module('app').controller('bm047_BieuDoTop10SanPhamBanThapController', function ($scope, $rootScope, $timeout, Notification, ComboboxDataService, baoCaoSanLuongDataService) {
    CreateSiteMap();
    let activelist = 0;
    function init() {
        initdate();
    ...
    function load_chart() {
        ...
        baoCaoSanLuongDataService.getBieuDoTop10SanPhamBanThap(fromdate, todate).then(function (result) {
            ...
            if (totaldata[0].length > 0) {
                ...
                $scope.options = {
                    title: {
                        ...
                        text: $.i18n('menu_bieudotop10sanphambanthap')
                    },
                    tooltips: {
                        callbacks: {
    ...
    function loadgrid() {
        ...
        let fromdate = kendo.toString($scope.obj_TuNgay, formatDateTimeFilter);
        let todate = kendo.toString($scope.obj_DenNgay, formatDateTimeFilter);
        baoCaoSanLuongDataService.getBaoCaoTop10SanPhamBanThap(fromdate, todate).then(function (response) {
            $scope.gridData = {
                data: response.data,
                schema: {
                    model: {
    ...
    $scope.xuatExcel = function () {
        ...
        let fromdate = kendo.toString($scope.obj_TuNgay, formatDateTimeFilter);
        let todate = kendo.toString($scope.obj_DenNgay, formatDateTimeFilter);
        baoCaoSanLuongDataService.getExcelTop10SanPhamBanThap(fromdate, todate).then(function (result) {
            if (result.flag)
