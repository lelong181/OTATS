angular.module('app').controller('addDonHangOTAController', function ($rootScope, $scope, $state, $stateParams, $timeout, Notification, donHangDataService, ComboboxDataService, printer) {
    CreateSiteMap();
    ...
    function initCombobox() {
        donHangDataService.getcombonhanvienlap().then(function (result) {
            ...
            $scope.hdvData = result.data;
            let truongnhom = false;
            $.each(result.data, function (index, item) {
                //if (item.idnv == $rootScope.UserInfo.iD_QuanLy && $rootScope.UserInfo.level != 1) {
                //    __idnhanvien = $rootScope.UserInfo.iD_QuanLy;
                //}
                if (item.tenDangNhap == $rootScope.UserInfo.username) {
                    __idnhanvien = item.idnv;
                    truongnhom = item.truongNhom;
                }
            })
            if (__idnhanvien > 0) {
                $timeout(function () {
                    ...
                    loadlistkhachhang();
                    $("#nhanvien").data("kendoComboBox").readonly(!truongnhom);
                }, 100);
            } else {
    ...
    $scope.hdvOnChange = function () {
        $scope.hdvselect = this.hdvselect;
        if ($scope.hdvselect != undefined)
            __idhdv = ($scope.hdvselect.idnv < 0) ? 0 : $scope.hdvselect.idnv;
        else
            __idhdv = 0;
    }
