angular.module('app').controller('addDonHangPCHLController', function ($rootScope, $scope, $state, $stateParams, $timeout, Notification, donHangDataService, ComboboxDataService, printer) {
    CreateSiteMap();
    ...
    function listColumnsgridhangchon() {
        ...
        dataList.push({
            ...
            template: '<button ng-click="addrow()" class="btn btn-link btn-menubar" title ="' + $.i18n('button_themhang') + '" ><i class="fas fa-plus-circle fas-lg color-infor"></i></button> ',
        ...
        dataList.push({ field: "giaLe", title: $.i18n('header_giabanle'), format: formatNumberInGrid($rootScope.UserInfo.dinhDangSo), attributes: { class: "text-right" }, headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, filterable: defaultFilterableGrid, width: "90px" });
        ...
        dataList.push({
            ...
            template: function (e) {
                if (e.ngay == '') {
                    ...
                    return kendo.toString(e.ngay, "dd/MM/yyyy")
                }
            },
            format: "{0:dd/MM/yyyy}",
    ...
    function dateEditor(container, options) {
        ...
        //input.data("kendoDatePicker").value(new Date());
    }
            ...
            if (validatethem()) {
                ...
                donHangDataService.themdonhangdichvu(data).then(function (result) {
                    if (result.flag) {
                        ...
                        console.log(result);
                        $state.go('editdonhangpchl', { iddonhang: result.data.iddonhang });
