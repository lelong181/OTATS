angular.module('app').controller('editDonHangPKDController', function ($rootScope, $scope, $state, $stateParams, $timeout, Notification, donHangDataService, ComboboxDataService, printer) {
    CreateSiteMap();
    ...
    function init() {
        ...
        donHangDataService.getTongSoDu(0).then(function (result) {
    ...
    function getquyen() {
        ...
        ComboboxDataService.getquyen(url).then(function (result) {
            ...
            if ($scope.permission.iD_ChucNang <= 0) {
                ...
                $location.path('/danhsachdonhangpkd')
            }
        });
    }
    ...
    function getChiTietDonHang() {
        ...
        donHangDataService.getById(__iddonhang).then(function (result) {
            $scope.nhanvien = result.data.nhanVien;
    ...
    function modifiedformfield(data) {
        ...
        $scope.dienThoai = (data.dienThoai.trim() == '') ? data.maKH : data.dienThoai;
        ...
        $scope.objprint = data;
        $scope.objprint.nhanvien = $scope.nhanvien;
        $scope.objprint.tenTrangThaiThanhToan = $scope.nametrangthaithanhtoan;
        if ($scope.objectDonHang.conLai > 0) {
            let img = '<label style="width:100%">Thanh toán ck trực tiếp</label><img style="width: 190px; margin: auto;" src="https://img.vietqr.io/image/' + $scope.nhanvien.chucVu + '-' + $scope.nhanvien.queQuan + '-compact2.png?amount='
                + $scope.objectDonHang.conLai
                + '&addInfo=' + $scope.objectDonHang.maThamChieu
                + '"/>';
            $("#qrthanhtoan").html(img);
            let onepay = '<label style="width:100%">Thanh toán qua cổng Onepay</label><a href="' + SERVERIMAGE + 'Booking/ProcessPayment?iddh=' + $scope.objectDonHang.iD_DonHang + '"><img style="width: 160px;margin: auto;margin-top: 10px;" src="https://play-lh.googleusercontent.com/mvedVCbQg6ADKUYYraVLOlmOfOy2Rz66kEPvbmxt5xZ2TTa90Go9jBD2dJrwWmEo5g8"/></a>';
    ...
    function listColumnsgrid() {
        ...
        dataList.push({
            ...
            template: function (e) {
                if (e.mahang == 'TCVTNL') {
                    let count = e.dschitietmathang.some(function (st) { return st.maVeBoSung == "" || st.maVeBoSung == null || st.maVeBoSung == undefined });
                    if (count) {
                        return '<button ng-click="openformchitietve()" class="btn btn-link btn-menubar" title ="' + $.i18n('label_chonmathang') + '" ><i class="fas fa-tags fas-sm color-danger"></i></button> '
                    } else {
                        return '<button ng-click="openformchitietve()" class="btn btn-link btn-menubar" title ="' + $.i18n('label_chonmathang') + '" ><i class="fas fa-tags fas-sm color-infor"></i></button> ';
                    }
                } else {
                    return '<button ng-click="openformchitietve()" class="btn btn-link btn-menubar" title ="' + $.i18n('label_chonmathang') + '" ><i class="fas fa-tags fas-sm color-infor"></i></button> ';
                }
    ...
    function updatethanhtien() {
        ...
        let img = '<img style="width: 190px; margin: auto;"  src="https://img.vietqr.io/image/' + $scope.nhanvien.chucVu + '-' + $scope.nhanvien.queQuan + '-compact2.png?amount='
            + $scope.objectDonHang.conLai
            + '&addInfo=lsi'
            + kendo.toString($scope.objectDonHang.iD_DonHang, '000000')
            + '/>';
        $("#qrthanhtoan").html(img);
    }
    ...
    function listColumnsgridXuLy() {
        ...
        dataList.push({
            ...
            //    + '/>',
            title: 'Mã Booking', headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, filterable: defaultFilterableGrid, width: "150px"
        });
        ...
        dataList.push({
            ...
            template: function (e) {
                ...
                if ($scope.objectDonHang.iD_TrangThaiDongHang == 3 && e.iD_DichVu >= 1135 && e.iD_DichVu <= 1152) {
                    let link = "https://server.lscloud.vn/Booking/PrintQRTicketByBookingCodeForBaiDinh?ID_ChiTietDonHang=0&ID_DonHang="
                        + e.iD_DonHang
                        + "&ThuNgan=" + $scope.objectDonHang.tenNhanVien
                        + "&BookingCode=" + e.bookingCode
                    return "<a href='" + link + "' target='_blank'> <i class='fas fa-print'></i> In vé</a>"
                }
                else if ($scope.objectDonHang.iD_TrangThaiDongHang == 3) {
    ...
    function listColumnsgridchitietve() {
        ...
        dataList.push({
            field: "thoiGianSuDung", title: "Thời gian sử dụng",
        ...
        dataList.push({
            field: "ketQua", title: "Kết quả",
            attributes: { style: "text-align: center" },
            footerAttributes: { style: "text-align: right" },
            headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, width: "120px"
        });
        dataList.push({
            field: "groupLink", title: 'Vé điện tử',
            groupHeaderTemplate: function (e) {
                console.log(e);
                if (e.value != null) {
                    return "<a href='https://server.lscloud.vn/Booking/PrintTicketBySubBookingCode?ID_DonHang=" + e.items[0].iD_DonHang + "&GroupLink=" + e.value + "' target='_blank'> <i class='fas fa-link'></i> Link vé online (" + e.items.length  + " vé )</a> "
                } else {
                    return "Chưa tạo link"
                }
            },
            template: function (e) {
                if (e.groupLink) {
                    let link = "https://server.lscloud.vn/Booking/PrintTicketBySubBookingCode?ID_DonHang=" + e.iD_DonHang
        ...
        //dataList.push({
        //    field: "maVeBoSung", title: "Other Code",
        //    attributes: { style: "text-align: center" },
        //    footerAttributes: { style: "text-align: right" },
        //    headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, width: "120px",
        //    editor: function (container, options) {
        //        $('<input ng-keyup="keyupMaVeBoSung(input)" class="k-textbox" name="' + options.field + '"/>').appendTo(container)
        //    }
        //});
    ...
    function updateMaVeBoSung(model) {
        donHangDataService.checkTheDo(model.maVeBoSung).then(function (result) {
            if (!result.flag && model.maVeBoSung != "") {
                alert("Thẻ đò đã được sử dụng!");
            }
            else {
                donHangDataService.updateMaVeKhac(model.iD_ChiTiet_MatHang_DonHang, model.maVeBoSung).then(function (result) {
                    console.log(result)
                });
            }
        });
    ...
    function loadgridchitietve(_data) {
        $scope.gridchitietveOptions = {
            ...
            save: function (e) {
                if (e.values.maVeBoSung != undefined) {
                    e.model.maVeBoSung = e.values.maVeBoSung;
                    updateMaVeBoSung(e.model);
                }
                //console.log(e);
            }
        };
        ...
        $timeout(function () {
            ...
            //$timeout(function () {
            //    var cell = $("#gridchitietve td:eq(6)");
            //    $("#gridchitietve").data("kendoGrid").editCell(cell[0]);
            //}, 1000);
        }, 500);
    ...
    $scope.capnhatdsve = function () {
        commonOpenLoadingText("#btn_capnhatdsve");
        $("#btn_capnhatdsve").hide();
        donHangDataService.capnhatdsve($scope.objectDonHang.iD_DonHang).then(function (result) {
            if (result.flag) {
                getChiTietDonHang();
                Notification({ title: $.i18n('label_thongbao'), message: $.i18n('label_capnhatdonhangthanhcong') }, 'success');
            } else {
            ...
            $("#btn_capnhatdsve").show();
            commonCloseLoadingText("#btn_capnhatdsve");
        })
    }
    ...
    //        printer.printUrl("https://localhost:44368/Booking/PrintQRTicketByBookingCode?ID_ChiTietDonHang=13681&ID_DonHang=10839&ID_MatHang=199&ID_DichVu=17&BookingCode=6813719470", null);
    ...
    $scope.XuatExcel = function () {
        ...
        $("#gridchitietve").data("kendoGrid").saveAsExcel();
    }

    $scope.tachlinkve = function () {
        let grid = $("#gridchitietve").data("kendoGrid");
        let selectedItem = null;
        let arrid = [];
        $.each(grid.select(), function (index, item) {
            selectedItem = grid.dataItem(item);
            arrid.push(selectedItem.iD_ChiTiet_MatHang_DonHang);
        })
        ...
        text += kendo.toString(new Date(), 'HHmmss');
        donHangDataService.updateGroupLink({ listid: arrid, grouptext: text }).then(function (result) {
    ...
    $scope.thanhtoan = function () {
        ...
        commonOpenLoadingText("#btn_thanhtoanvi");
            ...
            donHangDataService.thanhtoan(obj).then(function (result) {
                if (result.flag) {
                    ...
                    if ($scope.objectDonHang.conLai == 0) {
                        ...
                        commonCloseLoadingText("#btn_thanhtoanvi");
                        getChiTietDonHang();
                    }
                    Notification({ title: $.i18n('label_thongbao'), message: $.i18n(result.message) }, 'success');
                } else {
    ...
    $scope.thanhtoanvi = function () {
        ...
        if (validateThanhToan()) {
            ...
            donHangDataService.thanhtoanvi(obj).then(function (result) {
                if (result.flag) {
                    loadgridThanhToan();
    ...
    $scope.giaHanVe = function () {
        donHangDataService.giaHanVe(__iddonhang, kendo.toString($scope.ngayGiaHan, "yyyy-MM-dd") + " 23:59:59").then(function (result) {
            console.log(result);
            if (result.data.success) {
                Notification({ title: $.i18n('label_thongbao'), message: result.data.msg }, "success");

                $scope.formdetailve.close();
            } else {
    ...
    function getHeaderXuLyDonHangDichVu(e) {
        ...
        let label = "<div style='width:100%;display:flex'> <p style='width:200px'>Thông tin thanh toán</p>"
            ...
            //+ "<p style='width:150px'> BookingCode: " + e.items[0].bookingCode + "</p>"
            + "<p style='width:150px'> Site: " + e.items[0].site.siteCode + "</p></<div>"
...
function keyupMaVeBoSung(input) {
    if (event.key === 'Enter' && $(input).val()) {
        let myGridAdd = $("#gridchitietve").data("kendoGrid");
        let data = myGridAdd.dataSource.data();
        console.log(data);

        let td = $(input).parent().parent().next().find("td");
        if (td.length > 0) {
            var existed = data.filter(function (st) { return st.maVeBoSung == $(input).val() });
            if (existed.length > 1) {
                $(input).val("");
            } else {
                $("#gridchitietve").data("kendoGrid").editCell($(input).parent().parent().next().find("td")[5]);
            }
        } else {
            alert("Đã đủ số lượng thẻ đò!");
        }
    }
}