...
angular.module('app').controller('posBanVeController', function ($rootScope, $scope, $state, $stateParams, $timeout, Notification, donHangDataService, ComboboxDataService, printer) {
    CreateSiteMap();
    let interval = null;
    let timeout = null;
    ...
    let __arrhttt = [
        ...
        { id: 2, name: "Thanh toán chuyển khoản VNPay" },
        { id: 3, name: "Thanh toan thẻ" },
    ]
    let __lsaccountcode = "";
    let __lsbookingcode = "";
    ...
    let __combotypeview = __arrtypeview[1];
    let __idkhuyenmai = 0;
    let __idnhanvien = 0;
    ...
    function initCombobox() {
        donHangDataService.getcombonhanvienlap().then(function (result) {
            ...
            if (__idnhanvien > 0) {
                ...
                $scope.nhanvienselect = $scope.nhanvienData[0];
                __idnhanvien = $scope.nhanvienselect.idnv
                loadlistkhachhang();
                loadhinhthucthanhtoan();
    ...
    function inittreeview() {
        ...
        $scope.selectOptions = {
            ...
            //change: onSelectNhom,
            open: function (e) {
                console.log($scope.dropdownHangChon.treeview);
                $scope.dropdownHangChon.treeview.expand(".k-item");
            },
            close: onSelectNhom
        };
    ...
    function setDataTreeview(data) {
        ...
        $(".k-check-all .k-checkbox-label").text("Chọn tất cả ngành hàng");
        loadgridhangchon();
    }
    ...
    function loadgridhangchon() {
        ...
        donHangDataService.getdanhsachmathangbyidkhachhangmultiidnhom(__idkhachhang, __idnhom).then(function (result) {
            ...
            let arr_hangchonchuachon = arr_hangchonall.filter((item) => {
                let html = item.tenHang.replace("Tràng An", "<b style='color:#40128B'>Tràng An</b>");
            ...
            $.each(arr_hangchonchuachon, function (index, item) {
    ...
    function onSelectNhom(e) {
        let newdata = $scope.dropdownHangChon.value().join(',');
        if (newdata != __idnhom) {
    ...
    function loadlistkhachhang() {
        ...
        donHangDataService.getlistkhachhangbyidnhanvien(__idnhanvien).then(function (result) {
            ...
            $scope.makh = '';
            $scope.dienthoai = '';
            $scope.khachhangselect = $scope.khachhangData[0];
            __idkhachhang = ($scope.khachhangselect.iD_KhachHang < 0) ? 0 : $scope.khachhangselect.iD_KhachHang;
        });
    }

    function loadhinhthucthanhtoan() {
        donHangDataService.gethttt(__idnhanvien).then(function (result) {
            $scope.HTTTData = result.data;
        });
    }
    ...
    function listColumnsgrid() {
        ...
        dataList.push({
            ...
            template: function (e) {
                if (e.isDichVu > 0 || e.mahang == "TCVTNL") {
        ...
        dataList.push({
            ...
            headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, filterable: false, width: "85px"
        });
        dataList.push({
            ...
            headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, filterable: false, width: "90px"
        });
        ...
        dataList.push({
            ...
            attributes: { style: "text-align: right; font-weight:700; color:red" }, footerAttributes: { style: "text-align: right" },
    ...
    function numberQtyEditor(container, options) {
        $('<input  name="' + options.field + '"/>')
            .appendTo(container)
            .kendoNumericTextBox({
                min: 1,
    ...
    function savechitietdonhang(e) {
        ...
        if (e.values.soluong != null) {
            ...
            //let myGridChiTiet = $("#grid").data("kendoGrid");
            //let hasTE = myGrid.dataSource.get('TCTCTENN');
            ...
            //    if (num > 0) {
            //        let dataItem = myGrid.dataSource.get('TCVTNL');
            //        let ddataItem = myGridChiTiet.dataSource.get('TCVTNL');
            //        try {
            ...
            //                    mahang: dataItem.maHang,
            //                    tenhang: dataItem.tenHang,
            //                    soluong: (num2 == 0) ? num : num + 1,
            //                    soton: dataItem.soLuongTon,
            //                    idhanghoa: dataItem.idMatHang,
            //                    isDichVu: dataItem.isDichVu,
            //                    chietkhautien: 0,
            //                    chietkhauphantram: 0,
            //                    tongtien: giaban * ((num2 == 0) ? num : num + 1),
            //                    giaban: giaban,
            //                    tongtienchietkhau: 0,
            //                    hinhthucban: __idhinhthucban,
            //                    hinhthucban_name: __namehinhthucban,
            ...
            //                myGrid.dataSource.remove(dataItem);
            //            } else {
            //                ddataItem.soluong = (num2 == 0) ? num : num + 1;
            ...
            //    //    let ddataItem2 = myGridChiTiet.dataSource.get('TCNNPTNL');
            //    //    let dataItem2 = myGrid.dataSource.get('TCNNPTNL');
            ...
            //    //} else {
            //    //    try {
            //    //        let dataItem2 = myGridChiTiet.dataSource.get('TCNNPTNL');
            ...
            //    //    let ddataItem2 = myGridChiTiet.dataSource.get('TCBTVN');
            //    //    let dataItem2 = myGrid.dataSource.get('TCBTVN');
            ...
            //    //} else {
            //    //    try {
            //    //        let dataItem2 = myGridChiTiet.dataSource.get('TCBTVN');
            //    //        myGridChiTiet.dataSource.remove(dataItem2);
            ...
            //    //    let ddataItem2 = myGridChiTiet.dataSource.get('TCPTVNTE');
            //    //    let dataItem2 = myGrid.dataSource.get('TCPTVNTE');
            ...
            //    //} else {
            //    //    try {
            //    //        let dataItem2 = myGridChiTiet.dataSource.get('TCPTVNTE');
            //    //        myGridChiTiet.dataSource.remove(dataItem2);
            ...
            //    //    let ddataItem2 = myGridChiTiet.dataSource.get('TCPTTCTE');
            //    //    let dataItem2 = myGrid.dataSource.get('TCPTTCTE');
            ...
            //    //} else {
            //    //    try {
            //    //        let dataItem2 = myGridChiTiet.dataSource.get('TCPTTCTE');
            //    //        myGridChiTiet.dataSource.remove(dataItem2);
    ...
    function updatethanhtien() {
        ...
        let img = '<img style="width: 155px; margin: auto;" src="https://img.vietqr.io/image/' + $scope.nhanvienselect.chucVu + '-' + $scope.nhanvienselect.queQuan + '-compact.png?amount='
            + $scope.tienthanhtoan
            + '&addInfo=lsi"/>';
        console.log(img);
        $("#qrthanhtoan").html(img);
        $("#qrthanhtoan").kendoTooltip({
            filter: "img",
            position: "top",
            content: function (e) {
                var target = e.target;
                return "<img style='width:380px;' src='" + target[0].currentSrc + "' />"
            }
        }).data("kendoTooltip");
    }
    ...
    function listColumnsgridchitiet() {
        ...
        dataList.push({
            field: "chieudai", hidden: true, title: $.i18n('header_kichthuoc'),
            editor: numberEditor,
            format: formatNumberInGrid($rootScope.UserInfo.dinhDangSo),
            attributes: { style: "text-align: right" }, footerAttributes: { style: "text-align: right" },
            headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, filterable: false, width: "100px"
        ...
        dataList.push({
            field: "soluong", hidden: true, title: $.i18n('header_soluong'),
            editor: numberEditor,
            format: formatNumberInGrid($rootScope.UserInfo.dinhDangSo),
            attributes: { style: "text-align: right" }, footerAttributes: { style: "text-align: right" },
            headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, filterable: false, width: "100px"
        ...
        dataList.push({
            field: "tong", hidden: true, title: $.i18n('header_tongsoluong'),
            editor: numberEditor,
            format: formatNumberInGrid($rootScope.UserInfo.dinhDangSo),
            attributes: { style: "text-align: right" }, footerAttributes: { style: "text-align: right" },
            headerAttributes: { "class": "table-header-cell", style: "text-align: center" }, filterable: false, width: "100px"
    ...
    $scope.luudonhang = function () {
        kendo.ui.progress($(".toolbarmenu"), true);
        document.addEventListener('keydown', (e) => {
            if (e.key.toLowerCase() === 's' && e.ctrlKey) {
                e.preventDefault();
            } else if (e.key.toLowerCase() === 'd' && e.ctrlKey) {
        ...
        setTimeout(function () {
            if (validatethem()) {
                ...
                let data = {
                    ...
                    hinhthucthanhtoan: $scope.HTTTSelect
                }
                console.log(data)
                ...
                donHangDataService.themvathanhtoandonhang(data).then(function (result) {
                    ...
                    kendo.ui.progress($(".toolbarmenu"), false);
                    document.addEventListener('keydown', (e) => {
                        if (e.key.toLowerCase() === 's' && e.ctrlKey) {
                            e.preventDefault();
                            $scope.luudonhang();
    ...
    $scope.addrow = function (e) {
        if (__idkhachhang == 0) {
            ...
            // nhập số lượng sau khi thêm sp
            let newitem = myGridChiTiet.dataSource.get(item.mahang);

            let td = $("#grid")
                .data("kendoGrid")
                .tbody
                .find("tr[data-uid='" + newitem.uid + "']").find("td")[4];
    ...
    $scope.openformchitietshortcut = function () {
        DATAITEM = $('#grid').data("kendoGrid").dataSource.get("TCVTNL");
        ...
        $("#formdetail").data("kendoWindow").wrapper.find(".k-window-action").css("visibility", "hidden");
    ...
    $scope.luuchitiet = function () {
        ...
        let max = 0;
        $.each(chitietdh, function (index, item) {
            if (item.mahang == "TCVTNL") {
                max = item.soluong;
            }
        })
        let myGridAdd = $("#gridchitiet").data("kendoGrid");
        ...
        console.log(max);
        if (data.length < max) {
            Notification({ title: $.i18n('label_thongbao'), message: "Số lượng thẻ chưa đủ" }, 'warning');
            return;
        }
    ...
    $scope.keyupMaTheDo = function (input) {
        ...
        if (event.key === 'Enter') {
            ...
            //} else {
            //    Notification({ title: $.i18n('label_thongbao'), message: "Số lượng thẻ không được vượt quá số lượng thuyền" }, 'warning');
            //}
            $scope.mathedo = "";
        ...
        setTimeout(function () {
            if (validatethem()) {
                ...
                donHangDataService.themdonhangdichvu(data).then(function (result) {
                    if (result.flag) {
                        donHangDataService.getQRDynamic($rootScope.UserInfo.danhSachNhom, result.data.iddonhang, $scope.tienthanhtoan).then(function (rs) {
                            console.log(rs);
                            $scope.base64qr = rs.data.invoice.qr.image;
                            //let onepay = '<img style="width: 100%; margin: auto;" src="data:image/png;base64,' + rs.data.invoice.qr.image + '"/>';
                            //$("#onepaythanhtoan").html(onepay);
                            $scope.linkqr = SERVERIMAGE + "/FileUpload/invoice_qr/" + rs.data.invoice.id + ".jpg";
                            $scope.infoqr = result.data;
                            let onepay = '<img style="width: 100%; margin: auto;" src="' + $scope.linkqr + '"/>';
                            $("#popup-qrthanhtoan").html(onepay);
                            $scope.qrpayment.center().open();
                            Notification({ title: $.i18n('label_thongbao'), message: result.message }, 'success');
                            timeout = setTimeout(function () {
                               interval = setInterval(function () {
                                    console.log("Check thanh toán");
                                   donHangDataService.getLichSuThanhToan(result.data.iddonhang).then(function (resulttt) {
                                       console.log(resulttt)
                                       if (resulttt.data[0].iD_DonHang == result.data.iddonhang && resulttt.data[0].soTien == $scope.tienthanhtoan) {
                                           $("#trangthaittqr").text("Đã thanh toán thành công!");
                                            donHangDataService.getChiTietDichVu(result.data.iddonhang).then(function (resultdv) {
                                                let bookingCode = "";
                                                console.log(resultdv)
                                                $.each(resultdv.data, function (index, item) {
                                                    if (item.bookingCode != null && item.bookingCode != "")
                                                        bookingCode = item.bookingCode;
                                                });
                                                window.open("https://server.lscloud.vn/Booking/PrintQRTicketByBookingCodeForBaiDinh?ID_DonHang=" + result.data.iddonhang
                                                    + "&ThuNgan=" + $("#nhanvien").data("kendoComboBox").text() + "&BookingCode=" + bookingCode
                                                    , "mozillaWindow", "popup");
                                                $scope.qrpayment.close();
                                                $scope.huydonhang();
    ...
    $scope.inmathanhtoan = function () {

        printer.printFromScope("/app/components/donhang/posbanve/inMaQRThanhToan.html", $scope);
    }

    $scope.clearTimeout = function () {
        console.log("clear check qr");
        clearInterval(interval);
    }

    $scope.inve = function (data) {
        //$.each(data.chitietdonhang, function (index, item) {
        //    console.log(item.dschitietmathang);
        //    if (item.dschitietmathang.length > 0) {
        //        if (item.dschitietmathang[0].maBookingDichVu != null) {
        //            mabooking = item.dschitietmathang[0].maBookingDichVu;
        //        }
        //    }
        //})
        //let link = "https://server.lscloud.vn/Booking/PrintQRTicketByBookingCode?ID_ChiTietDonHang="
        //    + data.iD_ChiTietDonHang
        //    + "&ID_DonHang=" + data.iD_DonHang
        //    + "&ID_MatHang=" + data.iD_MatHang
        //    + "&ID_DichVu=" + data.iD_DichVu
        //    + "&BookingCode=" + data.bookingCode;
        window.open("https://server.lscloud.vn/Booking/PrintQRTicketByBookingCodeForBaiDinh?ID_DonHang=" + data.iddonhang
            + "&ThuNgan=" + $("#nhanvien").data("kendoComboBox").text() + "&BookingCode=" + data.mabooking
            , "mozillaWindow", "popup")
        $scope.huydonhang();
    }
