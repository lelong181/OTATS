...
angular.module('app').controller('posBanVeTAGController', function ($rootScope, $scope, $state, $stateParams, $timeout, Notification, donHangDataService, ComboboxDataService, printer) {
    CreateSiteMap();
    ...
    function updatethanhtien() {
        ...
        $scope.tienphuthu = $scope.phuthutien + (tongtiendonhang - tongtienchietkhaumathang) * $scope.phuthuphantram / 100;
        $scope.tienthanhtoan = tongtiendonhang - $scope.tienchietkhau + $scope.tienphuthu;
        ...
        setTimeout(function () {
            if (validatethem()) {
                ...
                let data = {
                    ...
                    phuthuphantram: $scope.phuthuphantram,
                    phuthutien: $scope.phuthutien,
                    lydophuthu: $scope.lydophuthu,
                    idnhanvientao: 0,
                    kinhdo: 0,
                    vido: 0,
                    diachitao: "",
    ...
    $scope.phuthuphantramOnchange = function () {
        $timeout(function () {
            updatethanhtien();
        }, 100);
    }
    $scope.phuthutienOnchange = function () {
        $timeout(function () {
            updatethanhtien();
        }, 100);
    }
    ...
    $scope.inve = function (data) {
        ...
        window.open("https://server.lscloud.vn/Booking/PrintQRTicketByBookingCodeForBaiDinh?ID_ChiTietDonHang=0&ID_DonHang=" + data.iddonhang
            + "&ThuNgan=" + $("#nhanvien").data("kendoComboBox").text() + "&BookingCode=" + data.mabooking + "&SiteCode=TAG"
            , "mozillaWindow", "popup")
        $scope.huydonhang();
    }
